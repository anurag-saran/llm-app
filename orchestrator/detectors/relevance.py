...
class RelevanceDetector(BaseDetector):
    ...
    async def evaluate(self, llm_prompt, llm_response, metadata=None, mode=DetectorMode.model_output):
        ...
        logger.debug(f"Evaluating relevance on LLM response: {llm_response}")
        ...
        async with httpx.AsyncClient(timeout=None) as client:
            response = await client.post(
                settings.DETECTOR_API_RELEVANCE_URL,
                json={"text_a": text, "text_b": metadata.get("gt_answer"), "metadata": {}},
                headers=headers
            )
    ...
    async def health_check():
        async with httpx.AsyncClient(timeout=None) as client:
            try:
                response = await client.get(f"{settings.DETECTOR_API_RELEVANCE_URL}/health")
